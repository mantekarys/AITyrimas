...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        self.embedding_compression_1 = nn.Sequential(
            nn.Conv2d(
                in_channels=768, out_channels=64, kernel_size=1, stride=1, bias=False
            ),
        ...
        self.compression_2_and_linear = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(in_features=8*8*8, out_features=128, bias=False),
            nn.LayerNorm((128)),
            nn.GELU(),
        )
...
# shpae torch.Size([1, 4, 256, 384])     
# stack 4 16                             
# embeddings torch.Size([1, 4, 256, 384])
# channel torch.Size([1, 4, 384, 256])   
        ...
        res = res.reshape((shape[0], shape[1] * 64, 14, 14))
